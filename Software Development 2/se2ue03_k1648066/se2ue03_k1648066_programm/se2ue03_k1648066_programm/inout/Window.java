...
/**
 * Class allows for simple output of graphical objects on a window. Provides
 * several static methods to draw points, lines, circles and text. The method
 * {@link #clear()} will clear the window.
 */
public class Window {
	...
	/**
	 * Draws a line from position (x1, y1) to position (x2, y2) with given
	 * color.
	 * 
	 * @param x1
	 *            the x-coordinate of the first point
	...
	/**
	 * Draws a rectangle on position (x, y) with width w and height h with given
	 * color.
	 * 
	 * @param x
	 *            the x-coordinate of the position
	...
	/**
	 * Fills a rectangle on position (x, y) with width w and height h with given
	 * color.
	 * 
	 * @param x
	 *            the x-coordinate of the position
	...
	/**
	 * When called, waits for a mouse click in window and returns the position
	 * as result. Blocks until the mouse click occurs.
	 * 
	...
	/**
	 * Static inner class for the content pane of the frame. Overrides the paint
	 * method and draws the image object.
	 */
	...
	static {
		...
		image.getGraphics().fillRect(0, 0, DEFAULT_WIDTH, DEFAULT_HEIGHT);
		window.setSize(DEFAULT_WIDTH + 12, DEFAULT_HEIGHT + headerHeight + 12);
		window.addWindowListener(new Window.WindowClosingAdapter());
		window.setVisible(true);
	}

	/**
	 * Computes the ascent of the current font. Ascent is the distance between
	 * the baseline and the top border of the text.
	 * 
